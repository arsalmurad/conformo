...
mission, and a short list of non-negotiable invariants: integer money, zero
runtime dependencies in `core`/`formats`, deterministic output, no headless
browser in the PDF path, "never claim compliance, show it." Those are
enforced in review, and a PR that violates one will be asked to change
regardless of how good the rest of it is.

## Before you write code

1. **For a compliance-data change** (`packages/compliance-data`): every fact
   needs a source URL from a tax authority, ministry, or official EU page,
   never a vendor blog, law firm summary, or encyclopedia. A fact you can't
   source that way goes in with `status: "unverified"`, not omitted and not
   guessed. Run `npm run build:compliance-data && npm run readme:matrix`
   after editing any file under `packages/compliance-data/src/data/` — the
   README's compliance table is generated, never hand-edited.
2. **For a new invoice format or CIUS**, don't add a format-specific field to
   ...
   from the existing model or it doesn't ship.
3. **Money is always integer minor units.** If you touch anything that
...
- If you're touching `packages/pdf/src/pdfa.ts` or `packages/formats/src/cii.ts`'s
  ...
  it is.
- These anti-goals won't be reconsidered in a PR thread: double-entry
  accounting, payroll, expenses, inventory, CRM, time tracking, becoming an
  ERP, a mandatory account, a mandatory server, or any feature that needs a
  headless browser to produce a PDF.
